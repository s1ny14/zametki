# gui/app.py — ВЕРСИЯ ТОЛЬКО С ХЭШТЕГАМИ (#теги)
...
class NoteApp:
    ...
    def setup_ui(self):
        ...
        # ХЭШТЕГИ
        ttk.Label(form_frame, text="Хэштеги (#учеба #работа #дом):", background=BG_COLOR, font=('Segoe UI', 10)).pack(anchor="w", pady=(12, 3))
        self.tags_entry = ttk.Entry(form_frame, font=('Segoe UI', 11))
        self.tags_entry.pack(fill=tk.X, pady=2)
        ttk.Label(form_frame, text="Пиши через пробел или запятую", background=BG_COLOR, font=('Segoe UI', 9), foreground="gray").pack(anchor="w")
        ...
        # Кнопка добавления
        ttk.Button(form_frame, text="Добавить заметку", style='Pink.TButton', command=self.add_note).pack(pady=20, fill=tk.X)
        ...
        ttk.Label(search_frame, text="Поиск (текст или #тег):", background=BG_COLOR, font=('Segoe UI', 10)).pack(side=tk.LEFT)
        ...
        clear_btn = tk.Button(search_frame, text="Очистить", bg=DARK_PINK, fg="white", font=('Segoe UI', 9, 'bold'),
                             relief='flat', command=lambda: self.search_entry.delete(0, tk.END) or self.refresh_notes())
        ...
        ttk.Button(btn_frame, text="Удалить выбранное", style='Pink.TButton', command=self.delete_selected).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Обновить список", style='Pink.TButton', command=self.refresh_notes).pack(side=tk.LEFT)
    ...
    def open_detail_window(self, note):
        ...
        if note.tags:
            ...
            ttk.Label(win, text=tags_str, background=BG_COLOR, foreground=DARK_PINK, font=('Segoe UI', 11)).pack(anchor="w", padx=20, pady=5)
